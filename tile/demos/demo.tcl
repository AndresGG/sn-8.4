#
# demo.tcl,v 1.120 2009/01/03 18:46:13 jenglish Exp
#
...
catch { package require keynav }
catch { package require ttk::dialog }
catch { package require tile::harmonize }
...
proc spinbox.pane {f} {
    ttk::frame $f
    set l [list \
	January February March April May June July August September Banana]

    grid \
	[ttk::spinbox $f.tsbv -values $l -textvariable V(M) -wrap 1] \
	[spinbox $f.sbv -values $l -textvariable V(M) -wrap 1] \
    -sticky we ;

    grid \
	[ttk::spinbox $f.tsbl -from 0 -to 100 -wrap 1] \
	[spinbox $f.sbl -from 0 -to 100 -wrap 1] \
    -sticky we ;
...
if {[llength [info commands ttk::spinbox]]} {
    $nb add [spinbox.pane $nb.spinbox] -text "Spinboxes"
}
...
proc tree.pane {w} {
    ...
    bind $w.t <KeyPress-F6> [list unfoldTree %W]
...
variable states [list \
    ...
    background alternate invalid hover]
...
proc repeatDemo {} {
    ...
    catch { keynav::enableMnemonics $top }
...
catch { keynav::enableMnemonics $::ROOT }
